<?php
    ...
    function dco_add_credits()
    {
        echo "<!-- This site was designed and coded by Danny Cohen - dannycohen.design -->";
    }
    ...
    function dco_substitute_for_empty_post_thumbnail_id($metadata, $object_id, $meta_key, $single)
    {
        if ($meta_key == '_thumbnail_id' && empty($metadata)) {
            ...
            if (!empty($images)) {
                reset($images);
                $metadata = key($images);
                if (is_int($metadata) && $metadata > 0) {
                    add_post_meta($object_id, '_thumbnail_id', $metadata);
                } //And now update the post with the correct thumbnail id.
            }
        }
        return $metadata;
    }
